...
**Вывод:** Хотя Python и выполняет этап компиляции, он традиционно считается интерпретируемым языком, поскольку итоговое выполнение кода осуществляется интерпретатором. Это позволяет Python быть платформонезависимым и обеспечивает высокую гибкость при разработке.
...
**Заключение:** Аннотации типов в Python — это мощный инструмент для написания более надёжного и понятного кода, особенно в крупных проектах и командах.
...
### Что такое list comprehension (генератор списков)?
...
которое никогда не изменяется на протяжении его жизненного цикла и возвращается методом `__hash__()`, и может сравниваться с другими объектами (реализует метод `__eq__()`). 
...
Отображение (mapping) – это объект-контейнер, который поддерживает произвольный доступ к элементам по ключам и описывает все методы, описанные в абстрактном базовом классе 
...
Замыкания — мощный инструмент в Python, позволяющий создавать функции с сохраненным состоянием и обеспечивать доступ к данным из внешней области видимости. 
...
после которых следует объект-генератор, то данный генератор делегирует доступ к подгенератору, пока он не завершится (не закончатся его значения), после чего продолжает своё исполнение.
...
`Wheel`-файлы - это zip-архивы, содержащие установочные файлы для Python-пакетов, и могут содержать бинарные расширения (например, скомпилированные модули C), которые необходимо собрать и установить на целевой машине.
...
С помощью **Tkinter** вы можете создавать самые разные настольные приложения для Windows, Mac OS и Linux, такие как игры, калькуляторы, инструменты визуализации данных, редакторы изображений и многое другое. 
Приложения **Tkinter** управляются событиями, что означает, что приложение ожидает ввода данных пользователем и реагирует на такие события, как нажатия кнопок, выбор меню и ввод текста. 
**Tkinter** также обеспечивает поддержку различных концепций программирования с графическим интерфейсом, таких как управление компоновкой, обработка событий и объектно-ориентированное программирование.
...
- **Сложность изучения**:
    ...
    Django имеет довольно крутую кривую обучения, особенно для новичков в веб-разработке. Он предоставляет множество функций и компонентов, 
...
1. **Микросервисы и архитектура событийно-ориентированных систем**: Для построения микросервисной архитектуры или системы, основанной на событиях, может быть предпочтительнее использовать более легковесные фреймворки или инструменты, специализированные на этом виде разработки, такие как FastAPI или Node.js. В Джанго можно организовать событийную систему, но это потребует чуть больше усилий
...
3. **Голые API сервисы:** Сервисы, в которых не нужен никакой функционал из “коробки” джанго. 
    То есть, никаких админок и прочей шелухи, только ручки и ORM для взаимодействия с БД

## F, Q объекты, Annotate

1. **F-объект**:
    F-объект позволяет вам обращаться к значениям полей модели и использовать их в выражениях для фильтрации, обновления и аннотации без необходимости извлечения их из базы данных. 
    Он представляет собой ссылку на значение поля модели и может использоваться для сравнения, сложения и вычитания с другими полями или константами.
...
3. **Менеджер objects**:
    Менеджер objects предоставляет удобные методы для выполнения различных операций с моделями данных, таких как создание, обновление, удаление и выборка.
...
Важно понимать этапы жизненного цикла `QuerySet` для эффективной работы с базой данных и предотвращения потенциальных проблем с производительностью или утечками ресурсов.

## **`bulk_create, bulk_save`** - как устроены, как самому сделать и что просиходит на уровне SQL

1. **bulk_create()**:
   
    Метод `bulk_create()` позволяет создавать несколько объектов модели одновременно, что может быть гораздо быстрее, чем создание каждого объекта по отдельности с использованием метода `create()`.
...
2. **bulk_update()**:
    ...
    Метод `bulk_update()` позволяет обновлять несколько объектов модели одновременно, что может быть полезно, когда вы хотите обновить много объектов сразу, например, при массовой обработке данных.
