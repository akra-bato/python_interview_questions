...
**Python** — интерпретируемый язык программирования, поэтому коду не нужна компиляция. 
Он выполняется на ходу, но используется промежуточная форма, которая называется компиляцией байт-кода.
...
**==** - сравнение значений, **is** - сравнение адресов в памяти
...
Неизменяемость хеша: Метод __hash__() должен возвращать одно и то же значение каждый раз на протяжении всей жизни объекта.

Сравнимость: Объект должен реализовывать метод __eq__() для корректного сравнения с другими объектами.

Неизменяемость объекта: Объект не должен изменяться после создания, так как изменение состояния может привести к несоответствию хеш-значения.
...
Пользовательские объекты могут быть ключами, если они хешируемы. Это значит, что класс объекта должен правильно реализовывать методы __hash__() и __eq__(), и объект должен быть неизменяемым после создания.
