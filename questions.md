...
Он **_интерпретируемый_** и **_мультипарадигменный_**.  
Python направлен на повышение **_читаемости_** кода и **_производительности_** разработчика.  
Применяется в сферах: **_веб-разработка_**, **_машинное обучение_**, **_анализ данных_**, **_разработка прикладного ПО и игр_**.  
...
При этом подходе программист составляет **_последовательность действий_**, которые **_нужно выполнить_** компьютеру.   

**_Декларативный_** язык – это язык программирования, который назначает **_техническую реализацию_** системы или программы **_для достижения определенной цели_**, но не указывает конкретных шагов для ее выполнения.  
Примеры декларативных языков: **_SQL_** и **_HTML_**. 
...
- **Применение:** `Java` обычно используется для написания крупных приложений, а `Python` чаще всего используется для написания быстрого прототипирования и научных вычислений.  
Python часто используется для написания более простых и лаконичных программ из-за простоты написания кода. 
...
**Результат:** Копия полностью независима от оригинала.  
Изменение любого элемента в копии (даже глубоко вложенного) не затронет оригинал.
...
**_Аннотации типов НЕ выполняются во время работы программы._** Python игнорирует их при выполнении — это просто метаданные.

Аннотации проверяются **_только инструментами статического анализа_** (mypy, PyCharm, VS Code с Pylance) до запуска кода.
...
Последовательностью в Python называется итерабельный объект, который поддерживает эффективный доступ к элементам 
с использованием целочисленных индексов через специальный метод `__getitem__()` и поддерживает метод `__len__()`, который возвращает длину последовательности.
...
Две последовательности равны, если они имеют одинаковый тип, равную длину и  
соответствующие элементы обоих последовательностей равны.  
Последовательности одинаковых типов можно сравнивать.  
Сравнения происходят в лексикографическом порядке: сначала по длине последовательностей,  
если же их длины равны, то результат сравнения равен результату сравнения первых отличающихся элементов.
...
- **deque** (double-ended queue): Двухсторонняя очередь, обеспечивающая быструю вставку и удаление элементов с обоих концов. 
...
- **ChainMap:** Класс для объединения нескольких словарей или отображений в одно логическое представление. 
...
- **OrderedDict:** Подкласс словаря, запоминающий порядок добавления элементов. 
...
- **defaultdict:** Подкласс словаря, который предоставляет значение по умолчанию для несуществующих ключей, используя фабричную функцию.
...
- **UserDict, UserList, UserString:** Классы-обёртки для создания пользовательских классов, наследующих поведение стандартных `dict`, `list` и `str`. 
...
**Ключ.**  
Порядок следования ключей не гарантируется (гарантируется с версии 3.7).  
Для маленьких словарей порядок будет тот же, что и в объявлении.  
Для больших порядок зависит от расположения элементов в памяти.  
Особый класс `OrderedDict` учитывает порядок добавления ключей.
...
Для cопрограмм используются ключевые слова `async` и `await`.
...
Объект, который определяет логику при входе и выходе из блока кода `with`, называется контекстным менеджером.
...
**Контекстный менеджер** нужен, чтобы **гарантировать корректное освобождение ресурсов и правильную обработку транзакций**, даже если в коде возникнет ошибка.
...
with Session(engine) as session:
    ...
    user.name = "New name"
    # commit вызывается автоматически при успешном выходе
# session закрыта, транзакция завершена

```

**Свой контекстный менеджер на основе класса**
...
**Свой контекстный менеджер с библиотекой contextlib**
...
1. **Тип приложений**:
    ...
    - **FastAPI**: Фреймворк, ориентированный на создание API. Он специализируется на создании быстрых и эффективных веб-API с использованием асинхронного программирования.
2. **Синхронность и асинхронность**:
    - **Django**: Поддерживает синхронное выполнение запросов. Хотя в Django 3.1 добавлена поддержка асинхронных представлений, большинство инструментов и библиотек в Django все еще ориентированы на синхронную работу.
    - **FastAPI**: Полностью поддерживает асинхронное выполнение запросов. Это позволяет создавать быстрые и эффективные веб-API, особенно в случае больших нагрузок и масштабирования.
...
4. **Производительность**:
    - **Django**: Обеспечивает хорошую производительность, но может быть несколько менее эффективным в случае больших нагрузок из-за синхронного выполнения запросов.
    - **FastAPI**: Из-за асинхронной природы и использования более современных технологий обеспечивает высокую производительность, особенно при работе с асинхронными запросами.
...
## `select_related, prefectch_related, select_for_update`
    
`select_related()` и `prefetch_related()` - это методы оптимизации запросов к базе данных в Django, которые позволяют предварительно загружать связанные объекты, чтобы снизить количество запросов к базе данных.

Вот примеры использования `select_related()` и `prefetch_related()`:
...
1. **select_related()**:
    ...
    Этот код выполняет один SQL-запрос, чтобы выбрать все книги вместе с информацией об их авторах. `select_related('author')` указывает Django предварительно загрузить связанные объекты авторов.
...
2. **prefetch_related()**:
    ...
    Этот код также выполняет только один SQL-запрос, чтобы выбрать всех авторов вместе с их книгами. Метод `prefetch_related('book_set')` указывает Django предварительно загрузить связанные объекты книг для каждого автора. При этом, чтобы получить доступ к связанным объектам книг, используется атрибут `book_set`.
    
3. **select_for_update():**
    
    `select_for_update` - выборка строк из базы данных с **блокировкой строк для чтения и последующего обновления** (или удаления) этих строк в рамках одной транзакции. Проще говоря, он используется для обеспечения атомарности операций чтения и обновления (или удаления) данных в многопоточной или распределенной среде.
    ...
    3. Затем мы обновляем значения полей в выбранных строках и сохраняем изменения в рамках транзакции.
    
    Использование `select_for_update` важно в ситуациях, когда необходимо гарантировать, что выбранные строки не будут изменены другими процессами или потоками до завершения текущей операции. Это позволяет избежать гонки за ресурсами и обеспечивает согласованность данных.
...
`Request` -> `URL Dispatcher (Router)` -> `Views` -> `Service` -> `Repo` -> `Serializer` -> `Response`
...
- **Тяжеловесность**:
    ...
    Из-за большого количества встроенных функций и компонентов Django может показаться излишне тяжеловесным для небольших проектов или прототипов. 
    Некоторые разработчики предпочитают более легковесные фреймворки для таких задач.
    
- **Сложность изучения**:
    
    Django имеет довольно крутой кривой обучения, особенно для новичков в веб-разработке. Он предоставляет множество функций и компонентов, 
    которые могут быть сложными для освоения, особенно если вы только начинаете.
    
- **Монолитность**:
    
    Django предполагает определенную структуру проекта и методологию разработки, что может ограничивать гибкость и расширяемость приложений, 
    особенно если вам нужно интегрировать его с существующими системами или использовать другие подходы к разработке.
...
1. **Микросервисы и архитектура событийно-ориентированных систем**: Для построения микросервисной архитектуры или системы, основанной на событиях, 
    может быть предпочтительнее использовать более легковесные фреймворки или инструменты, специализированные на этом виде разработки, 
    такие как FastAPI или Node.js. В Джанго можно организовать событийную систему, но это потребует чуть больше усилий

2. **Проекты, требующие высокой производительности и масштабируемости**: Для проектов, требующих высокой производительности и масштабируемости, 
    особенно если они связаны с большим объемом запросов или операций в реальном времени, могут быть более подходящими альтернативами фреймворки, специализированные на быстрой и эффективной обработке запросов, такие как FastAPI, Node.js или Go

3. **Голые API сервисы:** Сервисы, в которых не нужен никакой функционал из “коробки” джанго. 
    Тоесть, никаких админок и прочей шелухи, только ручки и ORM для взаимодействия с БД

## F, Q objects, Annotat

1. **F-объект**:
    F-объект позволяет вам обращаться к значениям полей модели и использовать их в выражениях 
    для фильтрации, обновления и аннотации без необходимости извлечения их из базы данных. 
    Он представляет собой ссылку на значение поля модели и может использоваться 
    для сравнения, сложения и вычитания с другими полями или константами.
...
2. **Q-объект**:
    Q-объект позволяет вам строить сложные запросы с использованием логических операторов И, ИЛИ и НЕ. 
    Он полезен, когда вам нужно создать запрос с несколькими условиями, которые могут быть комбинированы по разным правилам. 
...
3. **Менеджер objects**:
    Менеджер objects предоставляет удобные методы для выполнения различных операций с моделями данных, 
    таких как создание, обновление, удаление и выборка. 
    Это базовый менеджер, который используется по умолчанию в моделях Django.
...
4. **annotate**:
    Метод annotate позволяет добавлять вычисляемые агрегированные значения к каждому объекту в запросе. 
    Это полезно, когда вам нужно выполнить агрегацию по значениям поля и добавить результаты в каждый объект результата запроса. 
    Тогда как `aggregate()` возвращает одно значение агрегации для всего набора данных.
...
> В Django `QuerySet` представляет собой объект, который предоставляет интерфейс для выполнения запросов к базе данных и извлечения данных из нее.
> 

Цикл жизни `QuerySet` включает в себя несколько этапов:

1. **Создание QuerySet**:
    `QuerySet` создается при вызове методов менеджера объектов модели, таких как `all()`, `filter()`, `exclude()` и других.

2. **Формирование запроса**:
    При создании `QuerySet` Django строит SQL-запрос, основываясь на вызванных методах и фильтрах, примененных к `QuerySet`. 
    В этом этапе формируются условия, выбираются поля, определяется порядок сортировки и другие параметры запроса.

3. **Выполнение запроса**:
    `QuerySet` **ЛЕНИВО** используется, например, в цикле или при вызове метода `list()`, 
    Django отправляет SQL-запрос к базе данных для извлечения данных. Результаты запроса возвращаются в виде объектов моделей или значений полей.
...
5. **Использование данных**:
    Полученные данные могут быть использованы в приложении, например, для отображения на веб-странице, 
    обработки в представлении или передачи в шаблон для отображения.

6. **Освобождение ресурсов**:
    После использования `QuerySet` его следует освободить, вызвав соответствующие методы, такие как `close()` или `delete()`, чтобы освободить ресурсы базы данных и избежать утечек памяти.

Цикл жизни `QuerySet` начинается с его создания и завершается после использования и освобождения его ресурсов. 
Важно понимать этапы жизненного цикла `QuerySet` для эффективной работы с базой данных и 
предотвращения потенциальных проблем с производительностью или утечками ресурсов.

## **`bulk_create, bulk_save`** - как устроены, как самому сделать и что просиходит на уровне SQL

1. **bulk_create()**:
    
    Метод `bulk_create()` позволяет создавать несколько объектов модели одновременно, что может быть гораздо быстрее, 
    чем создание каждого объекта по отдельности с использованием метода `create()`.
    
    Пример использования `bulk_create()`:
    
    ```python
    # Этот код создает несколько объектов модели MyModel и массово вставляет их в базу данных за один запрос.
    from myapp.models import MyModel
...
2. **bulk_update()**:
    ...
    Метод `bulk_update()` позволяет обновлять несколько объектов модели одновременно, что может быть полезно, 
    когда вы хотите обновить много объектов сразу, например, при массовой обработке данных.
    
    Пример использования `bulk_update()`:
    
    ```python
    # Этот код обновляет поле some_field для всех объектов модели MyModel, удовлетворяющих условию фильтрации, за один запрос.
    from myapp.models import MyModel
    ...
    > *Использование `bulk_create()` и `bulk_update()` может значительно повысить производительность вашего приложения, особенно при работе с большими объемами данных. Однако важно помнить, что эти методы могут быть менее гибкими и требуют осторожного использования, чтобы избежать потери данных или нарушения целостности.*
    >
...
1. **HAVING**:
    ...
    В SQL запросах `HAVING` используется для фильтрации группированных данных. В Django ORM аналогичный функционал можно получить, используя `annotate()` и `filter()`. Например, чтобы отфильтровать группы, у которых количество объектов больше определенного значения:
...
2. **DISTINCT**:
    ...
    В SQL `DISTINCT` используется для выбора уникальных значений из столбца. В Django ORM аналогичный функционал можно получить с помощью метода `distinct()`.
...
Этот код определяет модель `MyModel` с двумя полями: `name` и `age`. После выполнения команд `makemigrations` и `migrate`, 
эти изменения будут применены к базе данных, и таблица `myapp_mymodel` будет создана соответствующим образом.
...
Блокировки в базе данных используются для контроля доступа к ресурсам базы данных с целью предотвращения конфликтов и сохранения целостности данных. 
В Django вы можете использовать блокировки для защиты критических секций кода от параллельных операций записи.

2. **transaction.atomic()**:

`transaction.atomic()` - это декоратор или контекстный менеджер, который обеспечивает атомарность выполнения блока кода в рамках транзакции базы данных. 

Нужно для:

1. согласованность данных в базе данных
2. несколько операций должны быть выполнены атомарно
3. если одновременный доступ к ресурсам базы данных может привести к нежелательным последствиям

## Что такое Middleware, для чего, как реализуется

**Middleware** – особый объект, который обычно изменяет входящий запрос или исходящий ответ. Например, добавляет заголовки, делает предварительные проверки. 
Middleware нужен, когда требуется подвергнуть обработке все запросы приложения.

На уровне языка это объект с методами `process_request` и `process_response`. Методы должны вернуть принятый объект (запрос или ответ) для дальнейшей обработки или выкинуть исключение, если что-то не в порядке. В этом случае дальнейшая обработка прекращается.
...
- Основа Dependency Injection:
    ...
    Dependency Injection отделяет процесс создания объектов от их использования. Это означает, что объекты, зависящие от других объектов, получают их извне, вместо создания их самостоятельно.

**Примеры реализации DI:**
...
- Использовать ```EXPLAIN ANALYZE``` для получения плана выполнения запроса.
    Хорошо, когда часто встречается ```Index Scan```
    Много ```Seq Scan``` означает, что не хватает индексов.

- Посмотреть метрики: время ввода-вывода (I/O), использование CPU, памяти

2. Способы решения

- Добавить или пересмотреть индексы
    Нужно индексировать столбцы из ```WHERE, JOIN, ORDER BY``` и создавать их только там, где они реально нужны.
    (Каждый индекс замедляет операции ```INSERT, UPDATE, DELETE```)
    При использовании составных индексов сначала ставить тот, который фильтрует жестче и чаще.

- Избегать функций и вычислений над индексируемыми столбцами в условиях
    Если в ```WHERE``` есть ```LOWER(email)``` или ```EXTRACT(YEAR FROM order_date)```, 
    СУБД не сможет использовать индекс — ей придётся вычислить функцию для каждой строки. 

- Оптимизировать ```JOIN```. 
    Проверить, проиндексированы ли столбцы, по которым идёт соединение с обеих сторон. 
    Иногда помогает изменить порядок соединения таблиц (начать с меньшей). 
    Избегать ```CROSS JOIN``` — он порождает огромное количество строк.

- Уменьшить объем извлекаемых данных
    Заменить ```SELECT *``` на явный перечень нужных столбцов.

- Фильтровать данные как можно раньше
    Ставить условия в ```WHERE``` до операций ```GROUP BY``` или ```JOIN```, а не в ```HAVING```

- Пересмотреть подзапросы. 
    Коррелированные подзапросы (те, что выполняются для каждой строки внешнего запроса) — частая причина замедления. 
    Попробовать заменить их на ```JOIN```. Для проверки существования записей используйте ```EXISTS``` вместо ```IN```: 
    ```EXISTS``` останавливается при первом совпадении, а IN сначала материализует весь список. 
    Осторожно с ```NOT IN```: если в подзапросе есть ```NULL```, условие не вернёт ни одной строки. 

- Подумать о временных таблицах или CTE. 
    Если в запросе есть сложный промежуточный результат, который используется несколько раз, 
    материализация его во временную таблицу или CTE может улучшить производительность. 

- Проверить статистику. 
    Устаревшая статистика о распределении данных в таблицах может заставить оптимизатор выбрать неоптимальный план. 
    В некоторых СУБД есть команды для обновления статистики. 

- Рассмотреть партиционирование. 
    Если таблица очень большая, партиционирование может значительно ускорить запросы, 
    которые фильтруют данные по этому признаку. 

- Оптимизировать пагинацию. 
    Классическая пагинация с ```OFFSET``` медленна на больших смещениях, потому что СУБД читает и отбрасывает много строк. 
    В таких случаях применяют keyset-пагинацию: запоминают последнее значение ключа и продолжают от него.

- Убрать лишнее. 
    Проверить, нет ли в запросе ненужных ```ORDER BY, DISTINCT``` или избыточных соединений. 
    Иногда ```DISTINCT``` можно заменить на более эффективную логику (например, ```GROUP BY```)

3. Что еще проверить

- Конкуренция за ресурсы. 
    Если запрос медленный не сам по себе, а в производственной среде, 
    возможно, дело в высокой нагрузке: других активных транзакциях, блокировках. 

- Фрагментация. 
    Фрагментация индексов или таблиц (когда данные разбросаны по диску) тоже может замедлять чтение. 
    В некоторых СУБД есть механизмы дефрагментации. 

## Что такое СУБД, для чего используется? Назови примеры СУБД.

***СУБД (система управления базами данных)*** — это программное обеспечение, которое позволяет *создавать, хранить, изменять, удалять* и *извлекать* данные из базы данных, а также управлять доступом к ним и обеспечивать их целостность и безопасность.

**Основные задачи и сценарии использования:**

* Хранение и структурирование больших объёмов данных - примеры структур: таблицы, документы, графы и т. д..

* Быстрый поиск и фильтрация. 

* Обеспечение целостности данных. 
    Например, с помощью ограничений (уникальность, внешние ключи) СУБД не даст сохранить противоречивые данные.

* Поддержка транзакций и ACID-свойств. 
    Это важно для финансовых и других критичных операций: либо все изменения применяются, либо ни одно (гарантия согласованности).

* Контроль доступа и безопасность. 
    Можно разграничивать права пользователей: кто-то только читает, кто-то может менять или удалять данные.

* Резервное копирование и восстановление. 
    СУБД предоставляют штатные средства бэкапов и восстановления после сбоев.

* Параллельный доступ и управление блокировками. 
    Несколько пользователей или приложений могут работать с одними и теми же данными одновременно, не ломая их.

* Аналитика и отчётность. 
    Многие СУБД умеют эффективно выполнять сложные агрегации, соединения и аналитические запросы.

**Примеры СУБД и их типы**

***Реляционные СУБД (работают с таблицами, используют SQL)***

**PostgreSQL** — мощная открытая СУБД, поддерживает JSON, геоданные, сложные типы и расширения. 
Часто выбирают для серьёзных проектов.

**MySQL** — популярная открытая СУБД, широко используется в веб-разработке.

**Microsoft SQL Server** — корпоративная СУБД от Microsoft.

**Oracle Database** — мощная коммерческая СУБД для крупных корпоративных систем.

**SQLite** — лёгкая встраиваемая СУБД без отдельного сервера; 
    отлично подходит для мобильных приложений, тестов и небольших проектов 
    (её удобно использовать локально на планшете, например).

***Нереляционные (NoSQL) СУБД (для специфических задач и гибких структур)***

**MongoDB** — документоориентированная СУБД: данные хранятся в виде JSON-подобных документов; 
удобно, когда схема данных меняется или не фиксирована.

**Redis** — хранилище «ключ‑значение», часто используют как кэш, для очередей и быстрых счётчиков.

**Cassandra** — распределённая колоночная СУБД, рассчитана на высокую доступность и большие объёмы данных.

**Neo4j** — графовая СУБД, оптимальна для хранения и анализа связей (соцсети, рекомендации, графы зависимостей).

## Что такое транзакция. Какие у неё есть свойства (ACID)
...
Основные концепции (свойства) транзакции описываются аббревиатурой 
...
## Что такое уровни изоляции транзакций. Какие они бывают

**Read Uncommitted (чтение незафиксированных данных)**
Это самый слабый уровень.
**Суть:** транзакция может читать данные, которые ещё не зафиксированы другими транзакциями.
**Что предотвращает:** практически ничего.

**Какие проблемы возникают:**

- «Грязное» чтение (dirty read) — видишь промежуточные, ещё не подтверждённые изменения.
- «Грязная» запись теоретически тоже возможна (хотя на практике многие реализации блокируют её даже здесь).
На практике почти не используется в бизнес‑логике из‑за риска получить некорректные данные.

**Read Committed (чтение зафиксированных данных)**
Самый распространённый «базовый» уровень (например, по умолчанию в `PostgreSQL` и `SQL Server`).
**Суть:** видишь только зафиксированные данные на момент чтения; каждая отдельная операция чтения видит «свежий» срез данных.
**Что предотвращает:** «грязное» чтение и «грязную» запись.

**Какие проблемы остаются:**

- Невоспроизводимое чтение (non‑repeatable read): если прочитать одну и ту же строку дважды, между чтениями другая транзакция могла её изменить и зафиксировать — второй раз получишь новые данные.
- Возможны аномалии при конкурентных обновлениях, если логика не продумана.

**Repeatable Read (воспроизводимое чтение)**
**Суть:** все чтения в рамках одной транзакции видят один и тот же «снимок» данных — тот, который был зафиксирован на момент начала транзакции. Данные, изменённые другими транзакциями после этого момента, не видны.
**Что предотвращает:** невоспроизводимое чтение. Реализуется через **MVCC** (многоверсионное управление конкурентным доступом): каждая транзакция работает со своей версией строк.

**Какие проблемы остаются:**

- Фантомные чтения (phantom reads): при повторном выборе по условию могут появиться новые строки, вставленные и зафиксированные другими транзакциями (в некоторых СУБД Repeatable Read всё же предотвращает и их — зависит от реализации).

**Serializable (сериализуемость)**
Самый сильный уровень изоляции.
**Суть:** результат параллельного выполнения транзакций эквивалентен результату их строго последовательного (по одной) выполнения.
**Что предотвращает:** все аномалии конкурентного доступа: «грязные» чтения, невоспроизводимые чтения, фантомные чтения и любые состояния гонки.
**Как реализуется:**

- Последовательное выполнение (редко, только для очень быстрых транзакций).
- Двухфазная блокировка (2PL) — надёжно, но может сильно снижать производительность из‑за блокировок.
- Сериализуемая изоляция снимков состояния (SSI) — оптимистический подход: транзакции выполняются без блокировок, а перед фиксацией проверяется, не возникло ли несериализуемой ситуации; если да — транзакция откатывается.
...
## Что такое гонка транзакций (race condition)? Как предотвратить?

**Transaction race (race condition в транзакциях)** — это ситуация, когда результат выполнения нескольких конкурентных транзакций зависит от случайного порядка их исполнения, из‑за чего данные могут оказаться несогласованными.

**Простой пример**
Представь, что на счёте у пользователя 1000 ₽. Одновременно запускаются две транзакции:

T1: снять 500 ₽.
T2: снять 700 ₽.
Обе сначала читают баланс: каждая видит 1000. Потом каждая считает новый баланс:

T1: 1000 − 500 = 500
T2: 1000 − 700 = 300
Затем обе записывают результат. Допустим, T2 запишется второй — в базе останется 300. Но по факту сняли только 700, а не 1200. Итог: пользователь потерял 200 ₽, которых вообще не тратил. Это и есть потерянное обновление (lost update) — классическая аномалия из‑за race condition.

**Как предотвращать (практические способы)**

1. Атомарные обновления в одном запросе (самый надёжный способ):

`UPDATE accounts SET balance = balance - 500 WHERE id = 1 AND balance >= 500;`
Проверку и изменение делаем в одной атомарной операции — база сама гарантирует корректность.

2. Оптимистичная блокировка (версия строки):

- Храним `version` в строке.
- При обновлении пишем `SET version = version + 1 WHERE ... AND version = <старое значение>`.
Если `ROW_COUNT = 0`, значит, кто‑то уже обновил строку — перезапускаем логику.

3. Пессимистичная блокировка:

- `SELECT ... FOR UPDATE (PostgreSQL, MySQL)` или `WITH (ROWLOCK, XLOCK) (SQL Server)`.
- Блокируем строку сразу, чтобы никто другой не мог её менять, пока ты не завершишь транзакцию.

4. Высокий уровень изоляции:

- Например, `SET TRANSACTION ISOLATION LEVEL SERIALIZABLE` — база будет отклонять транзакции, которые могут привести к гонкам. Но возможны откаты и повторные попытки.

5. Уникальные ограничения и бизнес‑ключи:

- Для бронирований: уникальный индекс на (`resource_id, booking_status = 'active'`) или на (`resource_id, date`). Это не даст создать две активные брони на один ресурс.

## Что такое курсор и зачем он нужен

Запрос к реляционной базе данных обычно возвращает несколько рядов (записей) данных, но приложение за один раз обрабатывает лишь одну запись. 
Даже если оно имеет дело одновременно с несколькими рядами (например, выводит данные в форме электронных таблиц), их количество по-прежнему ограничено. 
Кроме того, при модификации, удалении или добавлении данных рабочей единицей является ряд. В этой ситуации на первый план выступает концепция курсора, и в таком контексте курсор – указатель на ряд.

**Курсор в SQL** – это область в памяти базы данных, которая предназначена для хранения последнего оператора SQL. 
Если текущий оператор – запрос к базе данных, в памяти сохраняется строка данных запроса, называемая текущим значением, или текущей строкой курсора. Указанная область в памяти поименована и доступна для прикладных программ.
...
Помимо того, что курсоры не позволяют проводить операции изменения над всем объемом данных, скорость выполнения операций обработки данных посредством курсора заметно ниже, чем у стандартных средств SQL.
...
**История**. PostgreSQL зародился в 1986 году в Калифорнийском университете, MySQL появился в 1995 году; с 2010‑го развивается под управлением Oracle.

**Хранение данных**. MySQL поддерживает разные движки (InnoDB, MyISAM и др.) — выбор движка влияет на поведение таблиц. PostgreSQL использует единый механизм хранения, таблицы реализованы как объекты с объектно‑ориентированными функциями.

**Соответствие SQL‑стандартам**. PostgreSQL строго следует стандартам SQL и поддерживает их новые версии. MySQL жертвует частью стандарта ради простоты и удобства.

**Обработка данных**. В MySQL результат запроса полностью загружается в память клиента — это может быть проблемой при больших объёмах. PostgreSQL работает через курсоры: клиенту передаётся указатель, а данные остаются на сервере (можно хранить курсор между сеансами). Плюс у PostgreSQL есть рекурсивные запросы, регулярные выражения, наследование таблиц.

**Производительность**. Изначально MySQL делали с упором на скорость (особенно по первичному ключу с InnoDB), PostgreSQL — на гибкость и соответствие стандартам. Сейчас PostgreSQL заметно нарастил производительность, но для простых операций чтения MySQL по‑прежнему часто быстрее.
...
**EXPLAIN** - это команда, которая показывает план выполнения запроса: какие операции собирается сделать СУБД, в каком порядке, какие индексы планирует использовать, сколько строк ожидает обработать и т. п. При этом сам запрос не выполняется.

В выводе обычно есть такие метрики:

- cost — оценочная «стоимость» операции (условные единицы);
- rows — ожидаемое количество строк;
- width — примерный размер строки в байтах.

Используется для быстрой проверки плана, диагностики без нагрузки

**EXPLAIN ANALYZE** выполняет запрос. Содержит прогноз и реальные метрики: `actual time` (реальное время), `actual rows` (фактическое число строк), `loops` (сколько раз выполнялся узел). Может нагружать БД; для `INSERT/UPDATE/DELETE` опасно без транзакции.

Используется для точного анализа узких мест, сравнения прогнозов с реальностью
